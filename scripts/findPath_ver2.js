...
// ====== GLOBALS for filtered rendering ======
...
function buildEndNodeCheckboxes() {
  ...
  if (
    ...
    mymiddleHarm.nodes.each(function (node) {
      ...
      var key = node.data && node.data.key !== undefined ? node.data.key : null;
      var label =
  ...
  displayedPaths = showAll
    ...
    : allPathsData.filter(function (it) {
        ...
        for (var i = 0; i < it.path.length; i++) {
          var n = it.path.get(i);
          if (n && n.data && filterEndKeys.indexOf(String(n.data.key)) >= 0)
            return true;
        }
        return false;
      });
...
// Highlight handler for the filtered list
...
// Returns probability of a middle-layer node by its key
...
// Returns vulnerability impact of a middle-layer node by its key
...
// Get list of currently checked end-node keys from the checkbox panel
function getCheckedEndFilters() {
  return Array.from(
    document.querySelectorAll("#endNodeFilters input[type=checkbox]:checked")
